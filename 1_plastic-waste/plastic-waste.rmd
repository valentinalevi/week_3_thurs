...
If colour function is within aes(), the argument changes based on the value of the variable (therefore changes for each continent in this case). Whereas outside aes(), it doesn't change based on the values of the variable (therefore affects them all).
...
ggplot(data = plastic_waste, 
  ...
  geom_point()
```
Slight positive correlation - the more plastic waste per captia the higher the amount of mismanaged plastic waste thre is.
...
ggplot(data = plastic_waste, 
       mapping = aes(x = mismanaged_plastic_waste_per_cap, 
                     y = plastic_waste_per_cap, colour = continent)) +
  geom_point()
```
 Europe - despite high levels of plastic waste per cap, there is still a low level of mismanaged plastic waste,
 Africa/Asia/Oceania - despite lower levels of plastic waste, a high level of it is mismanaged.
...
There is a slightly more linear correlation between plastic waste per capita and coastal population.
